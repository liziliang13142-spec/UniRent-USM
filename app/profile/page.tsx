...
export default function UserProfilePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase
      ...
      .upsert({
        ...
        matric_no: userDetails.matricNo, 
  ...
  if (!user) {
    return (
      // 🌟 修改点 1：去掉了 bg-gray-100，让加载界面变透明
  ...
  return (
    // 🌟 修改点 2：去掉了 bg-gray-50，让整个页面底层变透明
    <div className="min-h-screen pb-10">
      
      {/* 🌟 修改点 3：Header 变成半透明毛玻璃 bg-white/80 backdrop-blur-md */}
      ...
      <main className="p-4 max-w-md mx-auto mt-6">
        {/* 🌟 修改点 4：Card 变成半透明毛玻璃 bg-white/90 backdrop-blur-md */}
        <Card className="shadow-2xl border-none bg-white/90 backdrop-blur-md overflow-hidden rounded-2xl">
          {/* 顶部装饰条 */}
          <div className="h-24 bg-gradient-to-r from-blue-600/90 to-blue-400/90 w-full" />
            ...
            {isEditing ? (
              <form onSubmit={handleSubmit} className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <Input className="bg-white/50 border-gray-200 focus:bg-white" value={userDetails.username} onChange={(e) => setUserDetails({ ...userDetails, username: e.target.value })} />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-gray-400 uppercase ml-1">USM Matric No</label>
                    <Input className="bg-white/50 border-gray-200 focus:bg-white" value={userDetails.matricNo} onChange={(e) => setUserDetails({ ...userDetails, matricNo: e.target.value })} placeholder="e.g. 21xxxx" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-gray-400 uppercase ml-1">Phone Number</label>
                    <Input className="bg-white/50 border-gray-200 focus:bg-white" type="tel" value={userDetails.phoneNumber} onChange={(e) => setUserDetails({ ...userDetails, phoneNumber: e.target.value })} />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-gray-400 uppercase ml-1">Home Address</label>
                    <Input className="bg-white/50 border-gray-200 focus:bg-white" value={userDetails.address} onChange={(e) => setUserDetails({ ...userDetails, address: e.target.value })} />
                  </div>
                </div>
              ...
              <div className="space-y-5 animate-in fade-in">
                <div className="grid grid-cols-1 gap-4">
                  <div className="flex items-center gap-3 p-3 bg-blue-50/70 rounded-xl border border-blue-100/50">
                  ...
                  <div className="flex items-center gap-3 p-3 bg-gray-50/70 rounded-xl border border-gray-100/50">
                ...
                <div className="pt-2 space-y-2">
                  ...
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button variant="outline" className="w-full border-blue-200 text-blue-700 hover:bg-blue-50 rounded-xl h-12 font-semibold bg-white/50">Security Settings</Button>
                    </DialogTrigger>
                  ...
                  <Button variant="ghost" className="w-full text-red-500 hover:text-red-700 hover:bg-red-50/50" onClick={handleLogout}>Log Out</Button>
                </div>
              </div>
            )}
          </CardContent>
