...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 🌟 核心：我们丢掉所有花里胡哨的拦截，直接调用你写好的、完美的 login 函数连真实数据库！
      ...
      if (finalRole) {
        ...
        // 如果用户在白色的普通界面下
          ...
          if (finalRole.toLowerCase() === "admin") {
             ...
             router.push("/admin/dashboard") // 管理员从白屏进，也送去后台
      ...
      <div className={`w-full max-w-md rounded-2xl shadow-2xl p-8 border transition-all duration-500 ${isAdminMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-white/50'}`}>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-2">
            ...
            <Input
              type="text"
              placeholder={isAdminMode ? "Enter admin email" : "e.g. user@unirent.com"}
              value={username}
