...
export type Property = {
  ...
  bathrooms: number
  size: string
  area?: string // 🌟 修复 1：类型定义中必须包含 area
  description?: string
  landlordId?: string
  reviews?: Review[]
  views: number 
  ...
  const fetchProperties = async () => {
    ...
    if (error) {
      ...
      const formatted = data.map((p: any) => ({
        ...
        size: p.size,
        area: p.area, // 🌟 修复 2：从数据库读取时映射 area 字段
      ...
      .insert([
        {
          ...
          area: (newProperty as any).area, // 🌟 修复 3：插入数据库时包含 area 字段
  ...
  const updateProperty = async (updatedProperty: Property) => {
    const { error } = await supabase
      ...
      .update({ 
        ...
        area: updatedProperty.area // 🌟 确保更新时也会同步地区
      })
