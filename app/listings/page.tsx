...
export default function PropertyListingsPage() {
  ...
  const handleIncrementViews = async (id: any) => {
    ...
    // 🌟 第一重保险：强制转换为 Number 类型
    // Supabase RPC 非常严格，必须传数字，传字符串 ID 会导致函数找不到报错
    const numericId = Number(id);
    if (isNaN(numericId)) return;

    try {
      // 🌟 第二重保险：参数名必须是 'row_id'，与数据库函数定义一致
      const { error } = await supabase.rpc('increment_views', { row_id: numericId });
      
      if (error) {
        console.error("RPC Error (PGRST202?):", error.message);
      } else {
        // 同步本地状态：让用户点击后立刻看到访问量 +1，无需刷新页面
      ...
      console.error("Critical error in view tracking:", err);
    }
  };
  ...
  return (
    ...
    >
      <div className={`min-h-screen backdrop-blur-[2px] transition-colors duration-500 ${isLandlord ? 'bg-slate-900/30' : 'bg-blue-50/40'}`}>
        ...
        {/* 导航栏 */}
        <header className={`${isLandlord ? 'bg-slate-900' : 'bg-blue-600'} text-white shadow-md sticky top-0 z-40 transition-colors`}>
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              {isLandlord && (
                <Link href="/landlord/viewing-management">
                  <Button className="bg-emerald-500 hover:bg-emerald-600 text-white font-bold h-9 rounded-xl shadow-lg border border-emerald-400 flex items-center gap-2 px-4 transition-all active:scale-95">
              ...
              <div className={`w-9 h-9 rounded-full border-2 shadow-sm flex items-center justify-center text-white font-bold ${isLandlord ? 'bg-slate-800 border-slate-700' : 'bg-blue-500 border-blue-400'}`}>
        ...
        <main className="max-w-7xl mx-auto p-4 sm:p-6 mt-2">
          ...
          <div className={`flex flex-col xl:flex-row xl:items-center justify-between mb-8 gap-4 bg-white/95 p-6 rounded-2xl shadow-sm border backdrop-blur-md ${isLandlord ? 'border-slate-200' : 'border-blue-100'}`}>
            <div>
              ...
              <p className="text-gray-500 text-sm mt-1 font-medium">
                {isLandlord ? 'Explore housing performance near USM.' : `Found ${filteredProperties.length} matches for your campus life.`}
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full xl:w-auto">
               <div className="relative w-full sm:w-[250px]">
                  ...
                  <Input 
                    ...
                    className={`w-full pl-9 h-10 bg-gray-50 border-gray-200 rounded-xl text-sm transition-all focus-visible:ring-blue-500`}
                    value={searchTerm}
          ...
          {filteredProperties.length > 0 ? (
                ...
                <Card key={property.id} className={`border shadow-sm hover:shadow-xl overflow-hidden bg-white rounded-2xl flex flex-col transition-all group ring-1 ${isLandlord ? 'border-slate-200 ring-slate-900/5' : 'border-gray-100 ring-blue-500/5'}`}>
                  <CardContent className="p-0 relative flex-grow">
                    ...
                    <div className={`absolute top-3 left-3 text-white text-[11px] font-black px-2.5 py-1 rounded-md shadow-sm z-10 border ${isLandlord ? 'bg-slate-900 border-slate-700' : 'bg-blue-600 border-blue-500'}`}>
                      RM {property.price} <span className="text-[10px] font-medium opacity-80">/ mo</span>
                    </div>
                    ...
                    <div className="p-4 flex-grow flex flex-col">
                      ...
                      {/* 🌟 实时显示的访问量图标和数字 */}
                      <div className="flex items-center text-[10px] text-slate-400 font-bold mb-2">
                        <Eye className="w-3 h-3 mr-1 text-slate-400" />
                        {property.views || 0} views
                      </div>
                      
                      <p className="text-gray-500 text-xs flex items-center gap-1 mt-1 mb-4">
                        <MapPin className={`w-3.5 h-3.5 text-red-500`} /> {property.distanceToUSM}km from campus
                      </p>
                      
                      <div className="grid grid-cols-3 gap-2 mt-auto text-gray-600">
                        <div className="bg-gray-50 p-2 rounded-xl text-center border border-gray-100">
                          ...
                          <span className="text-[10px] font-bold">{property.bedrooms} Bed</span>
                        </div>
                        <div className="bg-gray-50 p-2 rounded-xl text-center border border-gray-100">
                          <Bath className="w-4 h-4 mx-auto text-gray-400 mb-1" />
                          <span className="text-[10px] font-bold">{property.bathrooms} Bath</span>
                        </div>
                        <div className="bg-gray-50 p-2 rounded-xl text-center border border-gray-100">
                          <Maximize className="w-4 h-4 mx-auto text-gray-400 mb-1" />
                          <span className="text-[10px] font-bold truncate block">{property.size}</span>
                        </div>
                      </div>
                    </div>
                  </CardContent>
                  ...
                  <CardFooter className="bg-gray-50 p-3 border-t border-gray-100 mt-auto">
                    <Link href={`/property/${property.id}`} className="w-full">
                      <Button 
                        className={`w-full text-white font-bold h-10 rounded-xl shadow-sm transition-all active:scale-95 ${isLandlord ? 'bg-slate-800 hover:bg-slate-900' : 'bg-blue-600 hover:bg-blue-700'}`}
            ...
            <div className="text-center py-24 bg-white rounded-3xl border border-dashed border-gray-200 shadow-sm mt-8">
              ...
              <Button variant="outline" className="mt-6 font-bold rounded-xl" onClick={() => {setSearchTerm(""); setPriceFilter("all"); setDistanceFilter("all")}}>
                Clear Filters
              </Button>
            </div>
          )}
