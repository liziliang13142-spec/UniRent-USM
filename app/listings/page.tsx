...
export default function PropertyListingsPage() {
  ...
  // 🌟 全局存储所有房源的真实平均评分映射，保证免去死码 4.8 假象
  const [ratingsMap, setRatingsMap] = useState<{ [key: number]: string }>({});

  const isLandlord = user?.role?.toLowerCase() === "landlord";

  // 🌟 核心获取：一次性查出系统所有评论，并在前端动态演算每个房源的真实平均得分
  useEffect(() => {
    const fetchAllRatings = async () => {
      try {
        const { data, error } = await supabase.from('reviews').select('property_id, rating');
        if (!error && data) {
          const scores: { [key: number]: number[] } = {};
          data.forEach(r => {
            if (!scores[r.property_id]) scores[r.property_id] = [];
            scores[r.property_id].push(Number(r.rating) || 0);
          });
          
          const resultMap: { [key: number]: string } = {};
          properties.forEach(p => {
            const pRatings = scores[p.id];
            if (pRatings && pRatings.length > 0) {
              const avg = pRatings.reduce((sum, val) => sum + val, 0) / pRatings.length;
              resultMap[p.id] = avg.toFixed(1);
            } else {
              resultMap[p.id] = "0.0"; // 默认没有评分显示 0.0
            }
          });
          setRatingsMap(resultMap);
        }
      } catch (err) {
        console.error(err);
      }
      ...
      <div className={`min-h-screen backdrop-blur-[2px] transition-colors duration-500 ${isLandlord ? 'bg-slate-900/30' : 'bg-blue-50/40'}`}>
        ...
        <main className="max-w-7xl mx-auto p-4 sm:p-6 mt-2">
              ...
              <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center">
                <div className="relative flex-grow w-full">
                  ...
                  <Input 
                    ...
                    className="pl-10 h-12 bg-gray-50 border-none rounded-2xl focus-visible:ring-blue-500 shadow-inner text-black"
                    value={searchTerm}
                ...
                <div className="flex flex-wrap gap-2 w-full lg:w-auto">
                  <Select value={areaFilter} onValueChange={setAreaFilter}>
                    <SelectTrigger className="w-[140px] h-12 rounded-2xl bg-white border-gray-100 shadow-sm font-bold text-black">
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button variant="outline" className="h-12 rounded-2xl border-dashed border-2 border-gray-200 hover:bg-gray-50 font-bold px-4 text-black bg-white">
                        <Filter className="w-4 h-4 mr-2" />
                        Facilities {selectedFacilities.length > 0 && `(${selectedFacilities.length})`}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-56 p-4 rounded-2xl shadow-2xl border-blue-50 bg-white text-black">
          ...
          {filteredProperties.length > 0 ? (
                  ...
                  <CardContent className="p-0 relative flex-grow">
                    ...
                    <div className="p-5 flex-grow flex flex-col">
                      <div className="flex justify-between items-start mb-2">
                        <Link href={`/property/${property.id}`} onClick={() => handleIncrementViews(property.id)}>
                          <h3 className="font-black text-slate-800 text-lg leading-tight line-clamp-1 text-left">{property.name}</h3>
                        </Link>
                      </div>
                      ...
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        {/* 🌟 核心改进：把固定写死的 4.8 换成了动态演算的真实平均评分ratingsMap */}
                        <div className="flex items-center text-[10px] font-bold text-amber-500 bg-amber-50 px-2 py-1 rounded-lg">
                          <Star className="w-3 h-3 mr-1 fill-amber-500" /> {ratingsMap[property.id] || "0.0"}
                        </div>
            ...
            <div className="text-center py-32 bg-white rounded-[40px] border border-dashed border-gray-200 shadow-inner mt-8 text-black">
