...
import { 
  ...
  ArrowDown, ArrowUp, CalendarClock, TrendingUp, AlertTriangle
} from "lucide-react"
...
export default function MarketAnalysisPage() {
  ...
  // 🌟 核心修复 2：使用 useCallback 包裹核心逻辑，加入严密的 try/catch/finally
  const fetchAnalysisData = useCallback(async () => {
    ...
    try {
      ...
      if (propIds.length === 0) {
        ...
        return; // finally 块会自动执行 setLoading(false)
      }
      ...
      const details = bookings?.map(b => {
        ...
        const orderDate = b.created_at ? new Date(b.created_at) : new Date() 
      ...
      setErrorMsg(err.message || "Network Error: Failed to calculate KPIs.")
    } finally {
      // 🌟 不管发生什么，永远保证 loading 被解除！
      setLoading(false)
    }
  }, [user])
  ...
  useEffect(() => {
    if (user) {
      ...
      // 🌟 核心修复 3：强制兜底机制，就算 AuthContext 丢了 user，3秒后强制停止转圈
      const timer = setTimeout(() => setLoading(false), 3000)
      return () => clearTimeout(timer)
    }
  }, [user, fetchAnalysisData])
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-slate-50">
      ...
      <p className="text-slate-500 font-black text-sm tracking-widest uppercase animate-pulse">Calculating KPIs...</p>
    </div>
  )

  // 🌟 如果查询发生了报错，优雅地显示错误而不是卡死
  if (errorMsg) return (
    <div className="h-screen flex flex-col items-center justify-center bg-slate-50">
      <AlertTriangle className="h-12 w-12 text-red-500 mb-4" />
      <p className="text-slate-800 font-black text-lg">Failed to Load Dashboard</p>
      <p className="text-slate-500 text-sm mt-2">{errorMsg}</p>
      <Link href="/landlord/properties"><Button className="mt-6">Return to Properties</Button></Link>
    </div>
  )
              ...
              {chartData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <XAxis dataKey="month" tickLine={false} axisLine={false} tick={{ fontSize: 12, fill: '#64748b', fontWeight: 'bold' }} dy={10} />
                    <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 12, fill: '#64748b' }} tickFormatter={(value) => `RM ${value}`} />
                    <RechartsTooltip cursor={{ fill: '#f8fafc' }} contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)', fontWeight: 'bold' }} formatter={(value: any) => [`RM ${value}`, 'Revenue']} />
