    ...
    >
      ...
      <div className="min-h-screen bg-slate-900/40 backdrop-blur-[2px]">
        ...
        <main className="max-w-7xl mx-auto p-6 mt-4 space-y-6">
          ...
          {/* 🌟 3. 操作区 - Market Analysis 和 Add 按钮 */}
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white/95 p-8 rounded-[2.5rem] shadow-2xl border border-slate-200 backdrop-blur-md">
            ...
            <div className="flex flex-row gap-4">
              {/* Market Analysis 按钮 (进入详细 KPI 页面) */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {myProperties.map((property) => (
                <Card key={property.id} className="group border-none shadow-xl overflow-hidden bg-white rounded-[2.5rem] flex flex-col transition-all hover:shadow-2xl hover:-translate-y-2 backdrop-blur-sm relative">
                  ...
                  <CardFooter className="bg-slate-50/80 p-5 flex gap-3 border-t border-slate-100 backdrop-blur-sm">
                    <Link href={`/landlord/edit-property/${property.id}`} className="flex-1">
                      <Button variant="outline" className="w-full border-slate-200 font-black text-[10px] h-12 rounded-2xl bg-white hover:bg-slate-900 hover:text-white transition-all uppercase tracking-widest">
                        <Edit className="w-3.5 h-3.5 mr-2" /> Edit
                      </Button>
                    </Link>
                    <Button 
                      variant="outline" 
                      className="flex-1 border-red-100 text-red-500 hover:bg-red-500 hover:text-white font-black text-[10px] h-12 rounded-2xl bg-white transition-all uppercase tracking-widest" 
...
// 简单的 Badge 替代品 (如果没有引入 shadcn Badge)
