...
export default function AdminDashboard() {
  ...
  // 🌟 保持你的代码结构：只解构存在于你的 AuthContextType 中的成员
  ...
  const [subTab, setSubTab] = useState("Tenant") // 控制 Member Registry 的子页签
  ...
  const fetchAdminData = useCallback(async () => {
    ...
    try {
      // 🌟 核心修复 A：移除 order('created_at')，解决 42703 报错
      ...
      // 获取预约 (🌟 核心修复 B：匹配你的真实表名 viewing_requests)
      const { data: viewingsData, error: viewingsError } = await supabase
        .from('viewing_requests')
        .select('*')

      // 获取用户信息 (🌟 核心修复 C：读取你 profiles 表的真实属性)
        ...
        return (
          <div className="space-y-8 animate-in fade-in duration-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <StatCard icon={<Users className="w-7 h-7"/>} label="Registered Users" value={profiles.length} color="text-orange-600" />
              <StatCard icon={<Activity className="w-7 h-7"/>} label="Server Status" value="ONLINE" color="text-emerald-500" />
            </div>
          </div>
        )
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-50">
                  {filteredUsers.length === 0 ? (
                    <tr><td colSpan={3} className="py-20 text-center text-slate-300 italic">No {subTab.toLowerCase()} records found.</td></tr>
                  ) : filteredUsers.map((p) => (
                ...
                <tbody className="divide-y divide-slate-50">
                  {viewings.map((v) => (
                    <tr key={v.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-8 py-5 text-right uppercase"><span className="bg-blue-50 text-blue-600 px-3 py-1 rounded-full text-[9px] font-black">{v.status}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        return (
          <div className="space-y-6 animate-in fade-in duration-500 text-left">
            ...
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100">
              <table className="w-full text-left">
                <thead className="bg-slate-50 text-[10px] text-slate-400 uppercase font-black tracking-widest">
                  <tr><th className="px-8 py-5">Order ID</th><th className="px-8 py-5">Property</th><th className="px-8 py-5 text-right">Revenue</th></tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {bookings.map((b) => (
                    <tr key={b.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-8 py-5 text-right font-black text-slate-800 uppercase">RM {b.total_price}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
